    ...
    <div className="p-4 md:p-8 pb-28 md:pb-10">
      ...
      <AnimatePresence>
        {modalCerrar && turnoCerrar && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto font-[family-name:var(--font-sans)]" onClick={() => { setModalCerrar(false); setTratamientos([{ nombre: '', precio: 0, moneda: 'ARS' }]); setPagos([{ monto: 0, moneda: 'ARS', metodo: 'efectivo' }]); setTurnoCerrar(null); setComentarioClinico(''); }}>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => { setModalCerrar(false); setTratamientos([{ nombre: '', precio: 0, moneda: 'ARS' }]); setPagos([{ monto: 0, moneda: 'ARS', metodo: 'efectivo' }]); setTurnoCerrar(null); setComentarioClinico(''); }} className="text-slate-400 hover:text-slate-600 cursor-pointer p-1 rounded-lg">
                  <span className="material-symbols-rounded">close</span>
                </button>
              </div>
              ...
              <div className="flex gap-3 mt-8">
                <button
                  onClick={() => { setModalCerrar(false); setTratamientos([{ nombre: '', precio: 0, moneda: 'ARS' }]); setPagos([{ monto: 0, moneda: 'ARS', metodo: 'efectivo' }]); setTurnoCerrar(null); setComentarioClinico(''); }}
